<#
.SYNOPSIS
This cmdlet is used for removing a malicious or spam email message from all inboxes the email was sent too. For Exchange Servers in the cloud this cmdlet assumes you are in the United States. The -ConnectionUri parameter will vary for certain countries such as Germany and may require modifying this cmdlets -ConnectionUri default value.
...
# This example finds every email received from April 13 2020 with the Subject Action Required and removes it from every ones inbox. The OnPremise parameter specifies that your Exchange server is managed on site. As such the -ConnectionUri parameter will also need to be defined containing a link to your Exchange Server. This has not been tested so if you experience issues with this please inform me what they are.
