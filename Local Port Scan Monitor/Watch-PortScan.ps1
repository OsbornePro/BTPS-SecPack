...
Function Watch-PortScan {
    ...
    While ($True) {
        ...
        ForEach ($SourceAddress in $ArrayList) {
            ...
            # 4200000 miliseconds is 70 minutes
            # 2400000 miliseconds is 40 minutes
            # 1200000 miliseconds is 20 minutes
            ...
            If ($EntryObjList.Count -ge $Limit) {
                ...
                If ($EmailAlert.IsPresent) {
                    ...
                    If ($EntryObjList) {
                        ...
                        $EntryObjList | ForEach-Object {

                            $Section = @{
                              activityTitle = "An account failed to log on"
                              activitySubtitle = "$($_.Hostname)"
                              activityText  = "Failed Login Attempt reported by $($_.Hostname) at $($_.TimeCreated)"
                              activityImage = ""
	                          facts		  = @(
                                    @{
                                            name = "Detected On: "
                                    ...
                                    @{
                                            name = "Message"
                                            value = "Possible Port Scan Detected"
                                        }
                                )  # End Facts
                        ...
                        $Body = ConvertTo-Json -Depth 8 @{
	                        ...
	                        text  = "There were $($ArrayTable.Count) connections to $($EntryObjList[0].Hostname)in the last 20 minutes from $($EntryObjList[0].SourceAddress)"
	                        sections = $ArrayTable
                            potentialAction =   @(
                                            @{
                                                ...
                                                target = @("https://csrc.nist.gov/glossary/term/port_scan")
                                            },
                                            @{
                                                '@context'  = 'http://schema.org'
                                                '@type' = 'ViewAction'
