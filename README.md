...
If you wish to use Microsoft Teams for sending alerts instead of using email you can use the below code. It may take me a little while to implement this in the install script and in all of the alerts. Feel free to implement if you do not want to wait on me.

__HOW TO__ <br>
1. [Create an Incoming Webhook Connector on a Teams channel](https://docs.microsoft.com/en-us/microsoftteams/platform/webhooks-and-connectors/how-to/add-incoming-webhook)
2. Modify an alert script to include the below code. Remove any code used in sending an email alert.

```powershell
# POST AN ALERT TO TEAMS WITH POTENTIAL ACTION BUTTONS
If ($Results) {
    ...
    $Message = $Obj.Message
    $WebhookUrl = 'https://domain.webhook.office.com/webhookb2/<NumsAndChars>/IncomingWebhook/<NumsAndChars>/<NumsAndChars>' 
    # $WebhookUrl is the link generated by your incoming webhook from your Teams Channel
    $Body = ConvertTo-Json -Depth 8 @{
        ...
        potentialAction =   @(
                                @{
                                    ...
                                    '@type' = 'ViewAction'
                                    name = 'Read More Info'
                                    target = @("https://us-cert.cisa.gov/ncas/alerts/TA15-103A#:~:text=Because%20a%20zone%20transfer%20is%20a%20single%20query%2C,see%20CVE-1999-0532%20and%20a%202002%20CERT%2FCC%20white%20paper")
                                },
                                @{
                                    '@context'  = 'http://schema.org'
                                    '@type' = 'ViewAction'
                                    name = 'View SIEM For Correlation Events'
                                    target = @("https://siem-tool.domain.com/")
                                }
                            ) 
    }  # End Body
